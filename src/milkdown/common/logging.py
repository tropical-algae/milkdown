...
def get_logger() -> logging.Logger:
    logger_path = os.path.join(os.getcwd(), settings.LOG_PATH)
    logger_file = f'{settings.PROJECT_NAME.replace(" ", "")}-{time.strftime("%Y-%m-%d", time.localtime())}.log'
    os.makedirs(logger_path, exist_ok=True)
    
    logger_filepath = os.path.join(logger_path, logger_file)
    logging_config = get_system_logger_config(filename=logger_filepath)
